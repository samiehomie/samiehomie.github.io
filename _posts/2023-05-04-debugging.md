...
![debugger-extensions]({{ page.title }}/ext.png)
...
![debug-start]({{ page.title }}/debug-start.png)

## Run menu

최상위 메뉴 **Run**에는 가장 일반적인 실행 및 디버그 명령이 있다.

![debug-menu]({{ page.title }}/debug-menu.png)
...
![launch-configuration]({{ page.title }}/launch-configuration.png)

VS Code는 디버그 환경을 자동으로 감지하려고 하지만, 이 작업이 실패할 경우 수동으로 선택해야 한다.

![debug-environments]({{ page.title }}/debug-environments.png)
...
![launch-json-in-explorer]({{ page.title }}/launch-json-in-explorer.png)
...
![launch-json-intellisense]({{ page.title }}/launch-json-intellisense.png)
...
![add-config]({{ page.title }}/add-config.gif)
...
![breakpoints.png]({{ page.title }}/breakpoints.png)

`debug.showBreakpointsInOverviewRuler`설정을 활성화하여 에디터의 우측 개요 눈금자에 breakpoint를 표시할 수 있다.

![bpts-in-overview.png]({{ page.title }}/bpts-in-overview.png)
...
![log-points.gif]({{ page.title }}/log-points.gif)
...
![variables.png]({{ page.title }}/variables.png)
...
![watch.png]({{ page.title }}/watch.png)

**VARIABLES** 섹션에 포커스를 두고 입력하면 변수 이름과 값을 필터링할 수 있다.

![filtering-variables.png]({{ page.title }}/filtering-variables.png)
...
![launch-json-suggestions.png]({{ page.title }}/launch-json-suggestions.png)
